...
def generate(groups, strategy):
    ...
    # Build group initializer strings
    g_inits = []
    for gi, g in enumerate(groups):
        cpus = g["pus"]
        nc = len(cpus)
    ...
    # Multi-line macro with \ continuations
    def ml(s, indent=0):
        """Return a line with \ continuation, indented."""
        return " " * indent + s + " \\"

    macro = []
    macro.append(ml("#define TOPOLOGY_INIT"))
    macro.append(ml("{"))
    macro.append(ml(f".num_nodes = {nn},"))
    macro.append(ml(f".num_groups = {ng},"))
    macro.append(ml(".groups = {"))
    for i, gi in enumerate(g_inits):
        comma = "," if i < len(g_inits) - 1 else ""
        macro.append(ml(gi + comma, 8))
    macro.append(ml("},"))
    macro.append(ml(".nodes = {"))
    for i, ni in enumerate(n_inits):
        comma = "," if i < len(n_inits) - 1 else ""
        macro.append(ml(ni + comma, 8))
    macro.append(ml("}"))
    macro.append("    }")
